...
class Quotex:
    ...
    def __init__(
        ...
        # PIN code for the e-mail verification step Quotex sometimes asks
        # for at sign-in. Only read by pyquotex.http.login when the
        # platform actually requests it — see PinRequiredError.
        self.pin_code = None
        self.email_imap = None
        self.email_imap_password = None
        ...
        # IMPORTANT: pass self.resource_path so each Quotex(root_path=...)
        # instance reads/writes its own session.json. Without this, every
        # instance shares one global session.json regardless of root_path,
        # which silently corrupts logins across different accounts.
        session = load_session(user_agent, self.resource_path)
    ...
    def set_session(self, user_agent: str, cookies: str = None, ssid: str = None):
        ...
        self.session_data = update_session(session, self.resource_path)
    ...
    async def connect(self):
        ...
        # BUG FIX: repassa credenciais de PIN/IMAP para a nova instância de
        # QuotexAPI, caso contrário Login.awaiting_pin() nunca enxerga o PIN
        # definido pelo usuário via /pin e o login trava indefinidamente.
